...
class SessionManager:
    ...
    def _inject_storage_script():
        ...
        window.addEventListener('DOMContentLoaded', function() {
            const storedAuth = localStorage.getItem('serenai_auth');
            if (storedAuth) {
                try {
                    const authData = JSON.parse(storedAuth);
                    // Set a flag that Python can check
                    window.serenai_auth_data = authData;
                } catch (e) {
                    localStorage.removeItem('serenai_auth');
                }
            }
        });
        ...
        window.saveAuthData = function(authData) {
            localStorage.setItem('serenai_auth', JSON.stringify(authData));
        };
        ...
        window.getAuthData = function() {
            const stored = localStorage.getItem('serenai_auth');
